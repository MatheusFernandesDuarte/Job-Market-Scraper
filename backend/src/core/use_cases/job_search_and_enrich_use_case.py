# backend/src/core/use_cases/job_search_and_enrich_use_case.py
...
class JobSearchAndEnrichUseCase:
    ...
    def __init__(
        ...
        """
        self._search_service: JobSearchInterface = search_service
        self._page_scraper: PageScraperInterface = page_scraper
    ...
    async def execute(self, queries: list[str], max_results: int) -> list[JobPosting]:
        ...
        for i in range(0, len(filtered_pool), self._ENRICHMENT_BATCH_SIZE):
            ...
            tasks = [self._page_scraper.fetch_content(url=job.link) for job in batch]
