# backend/tests/services/google/test_service.py
...
def mock_client(mocker: MockerFixture) -> MagicMock:
    ...
    return mocker.patch("backend.src.services.google.service.GoogleCseClient", autospec=True).return_value


@pytest.fixture
def mock_from_google_item(mocker: MockerFixture) -> MagicMock:
    """Mocks the from_google_cse_item mapper function."""
    return mocker.patch("backend.src.services.google.service.from_google_cse_item")


@pytest.fixture
def mock_initial_filter(mocker: MockerFixture) -> MagicMock:
    """Mocks the InitialJobFilter class."""
    mock = mocker.patch("backend.src.services.google.service.InitialJobFilter", autospec=True)
